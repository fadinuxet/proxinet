...
class PutraceMapPage extends StatefulWidget {
  const PutraceMapPage({super.key});

  @override
  State<PutraceMapPage> createState() => _PutraceMapPageState();
}

class _PutraceMapPageState extends State<PutraceMapPage> with TickerProviderStateMixin {
  ...
  late PutracePresenceSyncService _presenceSync;
  ...
  void initState() {
    ...
    _presenceSync = GetIt.instance<PutracePresenceSyncService>();
    _getCurrentLocation();
    _loadNearbyStatus();
  ...
  Future<void> _enableNearbyDiscovery() async {
    try {
      ...
      //     precision: precision); // Removed unused variable
      // final expireAt = DateTime.now().toUtc().add(const Duration(minutes: 15)); // Removed unused variable
  ...
  void _loadAvailableUsers() {
    // For Flutter Map 8.x, we'll use a simpler approach
    // Load all available users but filter them based on current map center and zoom
    ...
    final mockAvailableUsers = <Map<String, dynamic>>[
      {
        'userId': 'demo_user_1',
    ...
    Future.delayed(Duration(milliseconds: 500), () {
      if (mounted) {
        setState(() {
          _availableUsers = mockAvailableUsers;
        });
      }
    });
    
    // Comment out the actual Firestore query
    /*
        ...
        FlutterMap(
          ...
          children: [
            TileLayer(
              ...
              userAgentPackageName: 'com.yallakoll.putrace',
            ),
            ...
            MarkerLayer(
              markers: [
                ...
                ..._nearbyUsers.asMap().entries.map((entry) {
                  ...
                  return Marker(
                    point: LatLng(user['lat'] ?? 0.0, user['lng'] ?? 0.0),
                    width: 32,
                    height: 32,
      ...
      children: [
        FlutterMap(
          ...
          options: MapOptions(
            ...
            initialZoom: 15.0,
            onMapReady: () {
              // Load available users when map is ready
              _loadAvailableUsers();
            },
            onPositionChanged: (position, hasGesture) {
              // Reload available users when map position changes (pan/zoom)
              if (hasGesture) {
                
                // Add a small delay to avoid too many reloads
  ...
  void _startChat(Map<String, dynamic> user) async {
    try {
      ...
      final conversationId = _getConversationId(currentUserId, user['id']);
      
      // Navigate to messages page with conversation context
      if (mounted) {
        context.push('/putrace/messages', extra: {
          'conversationId': conversationId,
          'userId': user['id'],
          'userName': user['name'] ?? 'User',
          'userType': user['type'] ?? 'unknown',
          'openChat': true, // Flag to indicate we want to open a specific chat
        });
      }
    } catch (e) {
      if (mounted) {
      ...
      children: [
        ...
        Row(
          children: [
                ...
                onPressed: () async {
                  if (_presenceSync.isAvailableForConnections) {
                    ...
                    await _presenceSync.setAvailabilityForConnections(
                      true,
                      audience: VisibilityAudience.everyone.name,
                      hours: 2,
                    );
                  }
                  if (mounted) setState(() {});
            ...
            Expanded(
              child: OutlinedButton.icon(
                onPressed: () => context.push('/putrace/availability'),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Visible to ${_formatAudience(_presenceSync.currentAudience)}',
                    style: TextStyle(
                      fontSize: 12,
                      color: scheme.onSurface.withValues(alpha: 0.7),
                    ),
  ...
  void _debugAvailabilityStatus() async {
    ...
    // final audience = await _presenceSync.getVisibilityAudience(); // Removed unused variable
...
- Current Audience: ${currentAudience ?? 'N/A'}
...
''';
  ...
  String _getMapViewInfo() {
    return 'Available users in current map view: ${_availableUsers.length} users';
  }
}
