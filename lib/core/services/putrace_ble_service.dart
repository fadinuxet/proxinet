...
class PutraceBleService {
  ...
  Future<void> startEventMode() async {
    try {
      ...
      final serviceGuid = Guid(PutraceBleConstants.serviceUuid);

      // Start advertising an ephemeral token for Putrace peers
      try {
        _discovery.add('Starting BLE advertising with service UUID: ${serviceGuid.toString()}');
        await _startAdvertising(serviceGuid);
        _discovery.add('✅ Started advertising Putrace service successfully');
      } catch (e) {
      ...
      try {
        ...
        await FlutterBluePlus.startScan(
          ...
          timeout: const Duration(seconds: 30), // 30-second scan cycles for battery optimization
        );
        _discovery.add('✅ Scanning for Putrace peers started successfully');

        // Add adaptive scanning to restart after timeout
        _startAdaptiveScanning();
        ...
        FlutterBluePlus.scanResults.listen((results) {
          ...
          for (final r in results) {
            try {
              ...
              _discovery.add('🔍 Device: ${r.device.platformName} (${r.device.remoteId})');
              ...
              final serviceGuid = Guid(PutraceBleConstants.serviceUuid);
              final hasService = ad.serviceUuids.contains(serviceGuid) ||
                  ad.serviceData.keys.contains(serviceGuid);
              
              if (!hasService) {
                _discovery.add('   ❌ Not a Putrace device (no matching service)');
                continue;
              }
  ...
  void _startAdaptiveScanning() {
    if (_isActiveScanning) return;
    _isActiveScanning = true;
    
    // Restart scanning every 2.5 minutes (30s scan + 2min pause)
    _scanTimer = Timer.periodic(Duration(minutes: 2, seconds: 30), (timer) {
      _restartScanning();
    });
  }
  ...
  void _restartScanning() async {
    try {
      ...
      _discovery.add('🔄 Restarted adaptive scanning cycle');
    } catch (e) {
      _discovery.add('Error restarting scan: $e');
    }
  }

  Future<void> stopEventMode() async {
    try {
      _isActiveScanning = false;
      _scanTimer?.cancel();
      if (!kIsWeb) {
        await FlutterBluePlus.stopScan();
      }
  ...
  Future<void> _startAdvertising(Guid serviceGuid) async {
    try {
      ...
      try {
        ...
        _discovery.add('Advertising Putrace token (config 1)…');
      ...
      try {
        ...
        _discovery.add('Advertising Putrace token (config 2)…');
        _setupTokenRotation(serviceGuid, advertiseSettings);
        return; // Success, exit early
      } catch (e) {
      ...
      try {
        ...
        _discovery.add('Advertising Putrace token (config 3)…');
        _setupTokenRotation(serviceGuid, advertiseSettings);
        return; // Success, exit early
      } catch (e) {
  ...
  void _setupTokenRotation(
    ...
    _tokenTimer = Timer.periodic(const Duration(minutes: 15), (_) async {
      try {
        ...
        _discovery.add('Rotated Putrace token');
      } catch (e) {
